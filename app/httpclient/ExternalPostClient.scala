...
@Singleton
class ExternalPostClient @Inject()(ws: WSClient, config: Configuration)(implicit ec: ExecutionContext) extends AbstractHttpClient(ws) with Logging {
    ...
    * @return Future of A
    */
    def get[A <: Object](apiPath: String, params: Seq[(String, String)] = Nil, extraHeaders: Seq[(String, String)] = Nil)
                        (implicit reads: Reads[A]): Future[A] = handleResponseEntity(super.get(buildRequestURL(apiPath), params, extraHeaders))
    ...
    * @return Future of A
    */
    def put[A <: Object](apiPath: String, requestBody: Option[JsValue] = None, params: Seq[(String, String)] = Nil, extraHeaders: Seq[(String, String)] = Nil)
                        (implicit reads: Reads[A]): Future[A] = handleResponseEntity(super.put(buildRequestURL(apiPath), params, requestBody, extraHeaders))
    ...
    * @return Future of A
    */
    def post[A <: Object](apiPath: String, requestBody: Option[JsValue] = None, params: Seq[(String, String)] = Nil, extraHeaders: Seq[(String, String)] = Nil)
                         (implicit reads: Reads[A]): Future[A] = handleResponseEntity(super.post(buildRequestURL(apiPath), requestBody, params, extraHeaders))
    ...
    * @return Future of A
    */
    def delete[A <: Object](apiPath: String, params: Seq[(String, String)] = Nil, extraHeaders: Seq[(String, String)] = Nil)
                           (implicit reads: Reads[A]): Future[A] = handleResponseEntity(super.delete(buildRequestURL(apiPath), params, extraHeaders))
    ...
    private def handleResponseEntity[A](response: Future[WSResponse])(implicit reads: Reads[A]): Future[A] = {
        response.map { response =>
            ...
            if (response.status >= 400) {
                throw new ExternalServiceException(response.status, "External Service Exception", Option(response.body[JsValue].as[ErrorResponse]))
            } else {
    ...
    private def buildRequestURL(apiPath: String): String = StringUtils.removeEnd(baseUrl, "/").concat("/").concat(StringUtils.removeStart(apiPath, "/"))
}
