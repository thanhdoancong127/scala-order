...
class AuthController @Inject() (components: SilhouetteControllerComponents)
   ...
   */
  def signUp: Action[AnyContent] = SecuredAction(WithRole[JWTAuthenticator]("Admin")).async { implicit request: Request[AnyContent] =>
   ...
   */
  def login: Action[AnyContent] = UnsecuredAction.async { implicit request: Request[AnyContent] =>
